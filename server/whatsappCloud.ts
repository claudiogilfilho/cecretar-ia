...
async function sendMedia(phoneNumberId: string, to: string, asset: { kind: string; url: string }) {
  ...
  return sendCloudRequest(phoneNumberId, { messaging_product: "whatsapp", recipient_type: "individual", to, type: kind, [kind]: { link: asset.url } });
}

async function processInboundMessage(message: InboundWhatsAppMessage) {
  if (message.type !== "text" || !message.text.trim()) return;
  const channel = await getWhatsAppChannelByPhoneNumberId(message.phoneNumberId);
  if (!channel || channel.status !== "connected") return;
  const existing = await findOpenWhatsAppConversation(channel.agentId, message.from);
  const conversationId = existing?.id ?? await createConversation(channel.agentId, message.contactName, "whatsapp", message.from);
  await appendConversationMessage({ conversationId, role: "lead", body: message.text, metadata: { externalMessageId: message.messageId, provider: "meta_cloud" } });
  ...
  await appendConversationMessage({ conversationId, role: "agent", body: reply.reply, mediaIntent: reply.mediaIntent, metadata: { source: reply.source, provider: "meta_cloud" } });
  ...
  await sendCloudRequest(channel.phoneNumberId!, buildTextPayload(message.from, reply.reply));
  if (reply.mediaIntent) {
    const asset = await getMediaAssetForIntent(channel.agentId, reply.mediaIntent);
    if (asset) await sendMedia(channel.phoneNumberId!, message.from, asset);
  }
}
...
export function registerWhatsAppWebhookRoutes(app: Express) {
  ...
  app.post(WHATSAPP_WEBHOOK_PATH, async (req: RawBodyRequest, res) => {
    ...
    try {
      const inbound = normalizeWhatsAppPayload(req.body);
      await Promise.all(inbound.map(processInboundMessage));
