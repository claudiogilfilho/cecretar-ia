...
export default function Home() {
  ...
  return <div className="min-h-screen bg-[#f7faf8]">
    ...
    <main className="lg:pl-[258px]"><header className="sticky top-0 z-10 flex h-[76px] items-center justify-between border-b border-[#e2ebe5] bg-white/90 px-5 backdrop-blur lg:px-9"><div><p className="text-sm text-muted-foreground">{config.company.name}</p><h1 className="font-[Manrope] text-xl font-extrabold">{navigation.find(item => item.id === section)?.label}</h1></div><Button variant="outline" onClick={() => setSection("simulador")}><PlayCircle className="mr-2 size-4 text-primary" />Testar agente</Button></header><div className="mx-auto max-w-[1480px] p-5 lg:p-9">
      ...
      {section === "agente" && <AgentEditor config={config} onSaved={refresh} />}
      {section === "simulador" && <Simulator config={config} onRefresh={refresh} />}
      {section === "conversas" && <InboxPage config={config} rows={conversations.data ?? []} onRefresh={refresh} />}
      {section === "midia" && <MediaLibrary config={config} onRefresh={refresh} />}
...
function Agenda({ config, rows, onRefresh }: { config: any; rows: any[]; onRefresh: () => void }) {
  ...
  return <div className="grid gap-6 xl:grid-cols-[320px_1fr] enter-up"><aside className="rounded-2xl border border-[#e3ece6] bg-white p-5"><p className="text-sm font-bold">Novo agendamento</p><div className="mt-5 space-y-4"><Field label="Nome"><Input value={name} onChange={e => setName(e.target.value)} /></Field><Field label="Data e hora"><Input type="datetime-local" value={date} onChange={e => setDate(e.target.value)} /></Field><Button className="w-full" onClick={() => { if (!name || !date) return toast.error("Informe nome e horário."); create.mutate({ agentId: config.agent.id, visitorName: name, scheduledFor: new Date(date).getTime() }); }}>Agendar</Button></div><div className="mt-6 rounded-xl bg-emerald-50 p-3 text-xs leading-5 text-emerald-800">Cada especialista poderá conectar seu Google Calendar e definir os horários disponíveis. A conexão OAuth real será ativada depois no painel.</div></aside><section className="rounded-2xl border border-[#e3ece6] bg-white"><div className="border-b p-5"><p className="text-sm font-bold">Agenda de {config.agent.name}</p></div>{rows.length ? rows.filter(row => row.agentId === config.agent.id).map(row => <div key={row.id} className="flex items-center gap-4 border-b p-5"><CalendarDays className="size-5 text-primary" /><div className="flex-1"><p className="text-sm font-semibold">{row.visitorName}</p><p className="text-xs text-muted-foreground">{formatDate(row.scheduledFor)}</p></div><Badge variant="secondary">{row.status}</Badge>{row.status !== "canceled" && <Button variant="ghost" size="sm" onClick={() => cancel.mutate({ appointmentId: row.id })}>Cancelar</Button>}</div>) : <Empty text="Nenhum agendamento deste especialista." />}</section></div>;
}
