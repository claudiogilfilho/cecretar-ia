...
function SimulatorPage({ config, onRefresh }: { config: any; onRefresh: () => void }) {
  ...
  const [ranScenarios, setRanScenarios] = useState<string[]>([]);
  const [approvedScenarios, setApprovedScenarios] = useState<string[]>([]);
  ...
  const scenarios = [
    { id: "preco", icon: MessageCircleMore, label: "Preço e capacidade", text: "Quanto custa uma sala para quatro pessoas?", criteria: "Informa que salas para até quatro pessoas partem de R$ 1.500, sem prometer disponibilidade." },
    { id: "localizacao", icon: PhoneCall, label: "Localização", text: "Onde fica o Duconde?", criteria: "Responde Rua Conde de Irajá, 910, Torre, CEP 50610100, sem criar dados adicionais." },
    { id: "midia", icon: ImageIcon, label: "Mídia", text: "Quero ver fotos das salas", criteria: "Associa a intenção de fotos ao histórico e só apresenta mídia cadastrada na biblioteca." },
    { id: "qualificacao", icon: UserRoundCheck, label: "Qualificação", text: "Meu nome é Ana, moro em Recife e preciso de uma sala para três pessoas.", criteria: "Registra nome, cidade e necessidade, fazendo apenas perguntas úteis para continuar." },
    { id: "humano", icon: UsersRound, label: "Transferência humana", text: `Quero falar com ${config.agent.transferKeyword}`, criteria: "Interrompe o bot e encaminha o histórico completo para atendimento humano." },
    { id: "agenda", icon: CalendarDays, label: "Agendamento interno", text: "Quero agendar uma visita na sexta-feira à tarde.", criteria: "Orienta o fluxo de visita conforme horário do Duconde, sem afirmar que uma vaga foi reservada." },
  ];
  ...
  const runScenario = (scenario: typeof scenarios[number]) => { setRanScenarios(current => current.includes(scenario.id) ? current : [...current, scenario.id]); handleSend(scenario.text); };
  const toggleApproval = (id: string) => setApprovedScenarios(current => current.includes(id) ? current.filter(item => item !== id) : [...current, id]);
  ...
  return <div className="grid gap-6 xl:grid-cols-[1fr_360px] enter-up">
    <section className="overflow-hidden rounded-2xl border border-[#e3ece6] bg-white"><div className="flex items-center justify-between border-b border-[#edf2ee] px-5 py-4"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-xl bg-emerald-50 text-primary"><Bot className="size-5" /></div><div><p className="text-sm font-bold">Simulador do agente</p><p className="text-xs text-muted-foreground">Canal de teste interno · Duconde Empresarial</p></div></div><Button variant="ghost" size="sm" onClick={reset}>Nova conversa</Button></div><AIChatBox messages={messages} onSendMessage={handleSend} isLoading={send.isPending} height="610px" placeholder="Escreva como um interessado responderia..." suggestedPrompts={["Quanto custa uma sala para 4 pessoas?", "Quero visitar o espaço na sexta-feira", "Onde fica o Duconde?", `Quero falar com ${config.agent.transferKeyword}`]} /></section>
    <aside className="space-y-5"><section className="rounded-2xl border border-[#e3ece6] bg-white p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-bold">Roteiro do piloto</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Execute cada cenário e marque “passou” somente se o critério de aceite for atendido.</p></div><Badge variant="secondary">{approvedScenarios.length}/6</Badge></div><div className="mt-4 space-y-3">{scenarios.map(scenario => { const Icon = scenario.icon; const ran = ranScenarios.includes(scenario.id); const approved = approvedScenarios.includes(scenario.id); return <div key={scenario.id} className={cn("rounded-xl border p-3", approved ? "border-emerald-200 bg-emerald-50" : "border-[#edf2ee]")}><div className="flex gap-3"><span className="grid size-8 shrink-0 place-items-center rounded-lg bg-slate-50 text-primary"><Icon className="size-4" /></span><div className="min-w-0 flex-1"><p className="text-xs font-bold">{scenario.label}</p><p className="mt-1 text-[11px] leading-4 text-muted-foreground">{scenario.criteria}</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" className="h-7 text-[11px]" onClick={() => runScenario(scenario)} disabled={send.isPending}>Testar</Button>{ran && <Button size="sm" variant={approved ? "default" : "ghost"} className="h-7 text-[11px]" onClick={() => toggleApproval(scenario.id)}>{approved ? "Passou" : "Marcar como passou"}</Button>}</div></div></div></div>; })}</div></section><section className="rounded-2xl border border-emerald-100 bg-emerald-50 p-5"><CheckCircle2 className="size-5 text-primary" /><p className="mt-3 text-sm font-bold text-emerald-950">Critério de aprovação</p><p className="mt-1 text-xs leading-5 text-emerald-800">O piloto estará pronto para o WhatsApp quando os seis cenários passarem sem informação errada, pergunta forçada ou transferência indevida.</p></section></aside>
  </div>;
}
